...
FLOWMAP <- function(mode = c("single", "multi", "one"), files, var.remove,
  ...
  print("unique(get.vertex.attribute(graph.xy, name = 'Timepoint', index = V(graph.xy)))")
  print(unique(get.vertex.attribute(graph.xy, name = "Timepoint", index = V(graph.xy))))
  graph.with.fixed.times <- ConvertOrigTime(graph.xy, orig.times)
  print("unique(get.vertex.attribute(graph.with.fixed.times, name = 'Timepoint', index = V(graph.with.fixed.times)))")
  print(unique(get.vertex.attribute(graph.with.fixed.times, name = "Timepoint", index = V(graph.with.fixed.times))))
