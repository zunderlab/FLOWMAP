...
clusters_full <- file.clusters$full.clusters
table_breaks   <- file.clusters$table.breaks # this will be used later in the annotation section
table_lengths <- file.clusters$table.lengths
# clusters_full <- c()
...
# print("final_edgelist_with_distances")
# print(final_edgelist_with_distances)
...
for (n in 1:(6-1)) {
  ...
  clusters <- rbind(file.clusters$cluster.medians[[n]],
                    file.clusters$cluster.medians[[n+1]])
  # print("clusters")
  # print(clusters)
  ...
  for (i in 1:nrow(components_mst_el)) {
    ...
    if (are.connected(output_graph,members_x[tmp_index[1]]+offset,members_y[tmp_index[2]]+offset)) {
      # cat(paste("\ntrue = ",i))
      E(output_graph,P=c(members_x[tmp_index[1]]+offset,members_y[tmp_index[2]]+offset))$label <- "MST"
    } else {
      output_graph <- add.edges(output_graph,c(members_x[tmp_index[1]]+offset,
              members_y[tmp_index[2]]+offset),weight=tmp_min,label="MST",sequence_assignment=n+1)
    }
  }
...
# for (i in 1:length(fcs_files)) {
  ...
  print("beep")
    ...
    rownames(medians) <- ((table_breaks[i]-table_lengths[i]) + 1):(table_breaks[i])
    print("eep")
    # anno_cat is all the annos concatenated, will be used to make "anno.Rsave" file
    anno_cat <- rbind(anno_cat, medians)
}
